...
- [2026-05-24 12:35:00 CEST]: Fix Upload Audio e Inserimento Trascrizione in Testo Cliente
  - *Details*: Risolto un bug critico per cui l'upload di file audio nei formati `.mp3` e `.m4a` falliva con errore "Formato audio non supportato" anche con file validi. La causa era che la validazione server-side si basava esclusivamente sul MIME type (`file.type`), il quale varia in base al browser e al sistema operativo (es. `.m4a` può essere `audio/mp4`, `audio/x-m4a`, `audio/x-aac` o stringa vuota). Inoltre, dopo una trascrizione riuscita, il testo trascritto viene ora inserito automaticamente nel campo "Testo cliente" del popup "Nuova richiesta", rendendo il contenuto visibile e modificabile dall'admin prima dell'invio.
  - *Tech Notes*:
    - **`src/lib/audio/elevenlabs-client.ts`**: Riscritta `validateAudioFile` per validare primariamente per **estensione file** (`.mp3`, `.m4a`, `.wav`) — affidabile su tutti i browser/OS — con fallback su MIME type per casi edge (es. Blob programmatici). Aggiunta la funzione helper `getFileExtension`. Esteso l'insieme dei MIME accettati con `audio/x-m4a`, `audio/x-aac`, `audio/aac`, `audio/wave` per coprire tutte le varianti note.
    - **`components/requests/request-form.tsx`**: Modificato `handleAudioUpload` per inserire il testo trascritto direttamente nel campo `requestText` (testo cliente) dopo una trascrizione riuscita, con separatore visivo `--- Trascrizione audio ---`. Aggiornato l'attributo HTML `accept` dell'input file per includere tutte le varianti MIME.
    - TypeScript: ✅ Zero errori (`pnpm tsc --noEmit`).

- [2026-05-24 12:39:00 CEST]: Pulsante e Modal Aggiornamento Generico per Richieste Manuali
  - *Details*: Al posto del pulsante 'Invia all'utente' nascosto in precedenza per le richieste manuali, è stato introdotto un nuovo pulsante 'Aggiorna preventivo'. Cliccandolo, si apre un modal per inserire un aggiornamento libero, che innesca lo stesso identico flusso di ricalcolo silente e redirect.
  - *Tech Notes*: Aggiunto uno stato `isGeneralUpdateOpen` in `ImportantQuestionsSection` per gestire l'apertura del nuovo modal. La logica di sottomissione asincrona e redirect è stata estratta in una funzione condivisa `submitUpdate(contentStr: string)`, chiamata sia da `handleAnswerSubmit` (per domande specifiche) che dal nuovo `handleGeneralUpdateSubmit`.
